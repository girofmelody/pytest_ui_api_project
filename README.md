...
Для UI
```bash
pytest tests/api --alluredir=allure-results/ui
```
И есть третий вариант запуск всех тестов:
```bash
...
- **Название теста**: Например, ` Поиск фильма 'Головоломка' и проверка результатов`.
